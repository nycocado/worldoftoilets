...
/**
 * Contém a lógica de negócio para aceitar uma denúncia de utilizador.
 */
@Injectable()
export class AcceptReportUserUseCase {
  constructor(
  ...
  /**
   * Aceita uma denúncia de utilizador e aplica soft delete no utilizador denunciado.
   *
  ...
  async execute(
    ...
    const userReported = report.userReported;

    await this.repository.accept(report, reviewer);
    await this.userService.softDeleteUser(userReported, reviewer);
